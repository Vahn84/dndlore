...
const LoreDetail: React.FC<{ isDM: boolean }> = ({ isDM }) => {
  ...
  const formatWorldDate = (
    ...
    const time = hasTime ? timeOfDayLabel(wd.hour ?? 0, wd.minute ?? 0) : "";
  ...
  const publishPage = async () => {
    try {
      ...
      if (type === "campaign" && (pageDraft as any)?.worldDate) {
        ...
        let groupId = groups && groups.length ? groups[0]._id : undefined;
        ...
        if (!groupId) {
          toast((t) => (
            <div>
              Published. No groups available to create a timeline event.
            </div>
          ));
  ...
  return (
    ...
    >
      ...
      >
        <div className="contentContainer">
          <h1 className="pageTitle">
            {isDM ? (
              <>
                {pageDraft.bannerUrl ? (
                  <div className="banner-icon loaded" onClick={removeBanner}>
                    <ImageIcon size={32} />
                ...
                setPageDraft((prev: any) => ({
                  ...
                  worldDate: parts
                    ? {
                        ...
                        monthIndex: parseInt(parts.monthIndex || "0", 10) || 0,
          ...
          {type === "campaign" && (pageDraft.sessionDate || isDM) && (
            <div className="dateWrapper">
              {isDM ? (
                <SessionDatePicker
                  value={(pageDraft as any).sessionDate || null}
                  onChange={(dateStr) =>
                    setPageDraft((prev: any) => ({
                      ...prev,
                      sessionDate: dateStr || "",
                ...
                <span className="sessionDate">
                  {pageDraft.sessionDate ? pageDraft.sessionDate : "N/A"}
                </span>
              )}
            </div>
          )}
          ...
          <div className="blocks">
            {(pageDraft as any)?.blocks?.map((b: any, i: number) => {
              ...
              return (
                <RichTextBlock
                  ...
                  onMoveUp={isDM ? () => moveBlock(i, -1) : undefined}
                  onMoveDown={isDM ? () => moveBlock(i, +1) : undefined}
                  onDelete={isDM ? () => removeBlock(i) : undefined}
                  placeholder={isDM ? "Add your content here..." : ""}
        ...
        {isDM && type === "campaign" && (
          <div className="metaFields" style={{ margin: "8px 0 16px" }}>
            <Divider />
