...
const DiscordEventModal: React.FC<DiscordEventModalProps> = ({
	...
	const [recapPageIds, setRecapPageIds] =
		useState<string[]>(initialRecapPages);
	...
	const buildDescription = useMemo(() => {
		...
		if (voiceChannel && guildId) {
			const selectedVoice = voiceChannels.find(
				(vc) => vc.id === voiceChannel
			);
			if (selectedVoice) {
				lines.push(
							...
							<div className="modal__column modal__column--right">
								...
								<div className="form-group">
									...
									{linkedEventId && (
										...
										>
											<span>
												TIMELINE: Auto-generated link to
												campaign group
											</span>
										</div>
									)}
									...
									<div style={{ marginBottom: '1rem' }}>
										...
										<input
											...
											value={webclientUrl}
											onChange={(e) =>
												setWebclientUrl(e.target.value)
											}
