  ...
  devtools(
    persist(
      immer((set, get) => ({
        ...
        isLoggedIn: () => get().user !== null && get().user?.token !== null,
        ...
        toggleGroup: (id: string) =>
          set((s) => {
            ...
            const next = has
              ? s.ui.activeGroupIds.filter((x) => (x === id ? false : true))
              : [...s.ui.activeGroupIds, id];
            // sanitize & dedupe
            s.ui.activeGroupIds = Array.from(
              new Set(
                next.filter((x): x is string => typeof x === "string" && !!x),
              ),
            );
          }),
        setGroupsFilter: (ids) =>
        ...
        loadGroups: async () => {
          ...
          try {
            ...
            set((s) => {
              ...
              const validIds = new Set(groups.map((g: Group) => g._id));
              s.ui.activeGroupIds = s.ui.activeGroupIds.filter((id) =>
                validIds.has(id),
              );
            });
          } catch (e: any) {
            set((s) => {
              s.data.groups.error = e?.message || "Failed to load groups";
            });
          } finally {
            set((s) => {
        ...
        loadEvents: async () => {
          ...
          try {
            ...
            set((s) => {
              s.data.events.error = e?.message || "Failed to load events";
            });
          } finally {
            set((s) => {
        ...
        loadPages: async (type) => {
          ...
          try {
            ...
            set((s) => {
              s.data.pages.error = e?.message || "Failed to load pages";
            });
          } finally {
            set((s) => {
        ...
        loadAssets: async () => {
          set((s) => {
            ...
            if (!s.data.assets) s.data.assets = initialLoadable<Asset[]>([]);
          ...
          try {
            ...
            set((s) => {
              s.data.assets.data = Array.isArray(list) ? list : [];
            });
          } catch (e: any) {
            set((s) => {
              s.data.assets.error = e?.message || "Failed to load assets";
            });
          } finally {
            set((s) => {
        ...
        deleteAsset: async (id) => {
          ...
          set((s) => {
            // @ts-ignore
            if (!s.data.assets) s.data.assets = initialLoadable<Asset[]>([]);
            s.data.assets.data = s.data.assets.data.filter((a) => a._id !== id);
          });
        },
        ...
        deleteGroup: async (id) => {
          ...
          set((s) => {
            s.data.groups.data = s.data.groups.data.filter((x) => x._id !== id);
        ...
        reorderGroups: async (ids) => {
          ...
          try {
            ...
            if (typeof anyApi.reorderGroups === "function") {
              ...
              await Promise.all(
                ids.map((id, idx) => Api.updateGroup({ _id: id, order: idx })),
              );
            }
          } catch (err) {
            // rollback on error
        ...
        deleteEvent: async (id) => {
          ...
          set((s) => {
            s.data.events.data = s.data.events.data.filter((x) => x._id !== id);
          });
          try {
        ...
        deletePage: async (id) => {
          ...
          set((s) => {
            s.data.pages.data = s.data.pages.data.filter((x) => x._id !== id);
          });
        },
        getPage: async (id) => {
        ...
        replacePageInCache: (p) => {
          set((s) => {
            const idx = s.data.pages.data.findIndex((x) => x._id === p._id);
            if (idx >= 0) s.data.pages.data[idx] = p;
            else s.data.pages.data.push(p);
          });
        },
        toggleLightRagDocument: async (
          pageId: string,
          action: "save" | "remove",
        ) => {
      ...
      })),
      {
        ...
        migrate: (state: any, version) => {
          ...
          if (state.ui && Array.isArray(state.ui.activeGroupIds)) {
            state.ui.activeGroupIds = state.ui.activeGroupIds.filter(
