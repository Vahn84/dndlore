...
class Api {
  ...
  static resolveAssetUrl(u: string) {
    u = process.env.NODE_ENV === "production" ? u : `http://localhost:3001${u}`;
    if (!u) return "";
  ...
  // -------------------------------------------------------------------------
  /**
   * Save page to LightRag or remove from LightRag
   */
  static async toggleLightRagDocument(
    pageId: string,
    action: "save" | "remove",
  ) {
